...
/*
* scanLines function 
* to scan lines and simulate the game.
* scoringBonus function call when new block fit perfect with the previous blocks.
* count line that get score.
*/
...
function scanLines() { 
	...
	if (LINES !== "") { 
		LINES_BLINK_TIMER = setInterval('blinkLines()', LINES_BLINK_DELAY); // Blinking lines to got whole line fill up.
...
/*
* blinkLines Function 
* lines that get fill up every grid will blink and it will get erace then score the points.
*/
function blinkLines() { 
	LINES_BLINK_COUNTER ++;
...
/*
* eraseLines function
* to erase lines that fill up with grids.
*/
function eraseLines() { 
	var lines = LINES.split(",");
	for (var i = 0, imax = lines.length; i < imax; i ++) { 
